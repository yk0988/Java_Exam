...
//메소드는 **정수 x**를 32비트 이진수로 출력합니다.
//>>> 연산자는 논리적 오른쪽 시프트로, 부호 비트와 관계없이 비트를 오른쪽으로 이동시킵니다.
//& 1을 통해 각 비트가 1인지 0인지 판별하고 출력합니다.
//비트는 32비트 크기로 출력되므로, 출력은 항상 32개의 비트로 이루어집니다.//
...
/*  이 프로그램은 사용자로부터 입력받은 정수 x에 대해 주어진 위치에서부터
    n개의 비트를 변경하는 여러 가지 기능을 제공합니다.
    setN으로 비트를 1로 설정하고, resetN으로 비트를 0으로 설정하며,
    inveresetN으로 비트를 반전시킵니다.
    각 연산의 결과는 32비트 이진수 형식으로 출력됩니다.*/
